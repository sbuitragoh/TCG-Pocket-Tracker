...
class CollectionViewer(tk.Tk):
    ...
    def _process_dataframe(self, df):
        ...
        if 'pack' in df.columns:
            ...
            df = df.with_columns(
                pl.col('pack').fill_null(fill_val)
            )
    ...
    def import_json(self):
        ...
        try:
            dfs = [self._process_dataframe(importer.read_json_file(path)) for path in file_paths]
    ...
    def _create_bottom_frame(self):
        ...
        inv_comp_frame = tk.LabelFrame(
            inv_frame, text = 'Inventory Completion', padx = 5, pady = 5, bg = '#e3f2fd',
        ...
        inv_pack_completion_frame = tk.LabelFrame(
            inv_frame, text = 'Inventory Pack Completion', padx = 5, pady = 5, bg = '#e3f2fd',
            fg = '#1565c0', labelanchor = "n")
        ...
        self.suggest_label = tk.Label(sug_lbl_frame, text="", font=("Arial", 12, 'bold'), fg="#1565c0")
        self.suggest_label.pack(fill=tk.BOTH, expand=True)

        self.suggest_label_set = tk.Label(sug_lbl_frame, text="", font=("Arial", 12, 'bold'), fg="#1565c0")
    ...
    def _show_tree(self, tree, df):
        ...
        checkbox_vars = self.checkbox_vars_set if self.is_set else self.checkbox_vars_all
        checkbox_vars.clear()
        ...
        for row in checked_data.iter_rows(named=True):
            values = [u"☑"] + [row[col] for col in df.columns if col != "row_idx"]
    ...
    def on_group_change(self, value):
        ...
        if value == "Checked":
            ...
            self.groups_all = self._populate_tree(self.tree, groups, self.checkbox_vars_all)
    ...
    def update_pack_suggestion(self):
        ...
        if not packs:
            self._set_suggest_label(
                "Congratulations! All packs are complete.")
            return
    ...
    # Ensure DataFrame has row_idx
        ...
        if pil_image:
            ...
            container_width, container_height = 480, 420
            original_width, original_height = pil_image.size
            scale = min(container_width / original_width, container_height / original_height)
            ...
            if idx not in self.inventory:
                ...
                image = pil_image.convert('L').resize(new_size, Image.Resampling.LANCZOS)
