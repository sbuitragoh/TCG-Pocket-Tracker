...
class CollectionViewer(tk.Tk):
    ...
    def _init_data(self, dataframe):
        ...
        self.unique_packs = dataframe['pack'].unique if 'pack' in dataframe.columns else []
    ...
    def _create_json_menu(self, menubar):
        ...
        json_menu.add_command(label = 'Fetch Card Images', command = self._show_download_progress)
        menubar.add_cascade(label = 'Dataset', menu = json_menu)

    def import_json(self):

        file_paths = filedialog.askopenfilenames( defaultextension=".json",
        ...
        try:
            dfs = [self._process_dataframe(importer.clean_db(importer.read_json_file(path))) for path in file_paths]
    ...
    def save_progress(self):
        ...
        file_path_str = filedialog.asksaveasfilename(  
            ...
            filetypes=[("Pokemon Inventory Files", "*.pif"),
                        ("All Files", "*.*")]  
        )
        ...
        try:
            ...
            if not getattr(self, 'json_path', None):
                self.json_path = pathlib.Path(resource_path('/sets/a3-celestial-guardians.json'))
    ...
    def _create_top_frame(self):
        ...
        # Filtrar "id" y "name"
        group_options = ["Checked"] + [col for col in self.df.columns if col not in ("id", "name")]
        self.group_var.set('Checked')
        group_menu = ttk.Combobox(
            top_frame, textvariable=self.group_var, values=group_options, state="readonly")
        group_menu.pack(side=tk.LEFT, padx=5)
        group_menu.bind("<<ComboboxSelected>>",
                        lambda e: self.on_group_change(self.group_var.get()))
    ...
    def _create_bottom_frame(self):
        ...
        set_comp_frame = tk.LabelFrame(
            set_frame, text = 'Set Completion', padx = 5, pady = 5, bg = '#e8f5e9',
            fg = '#1b5e20', labelanchor = 'n')
        set_comp_frame.pack(side = tk.TOP, fill = tk.BOTH, expand = False, padx = 2, pady = 2)
        self.set_comp_label = tk.Label(set_comp_frame, text = '', bg = '#e8f5e9',
            fg = '#1b5e20', font = ('Arial', 11, 'bold'), anchor = 'w', justify = 'left')
        self.set_comp_label.pack(anchor = 'w', padx = 5, pady = 2)

        set_pack_completion_frame = tk.LabelFrame(
            set_frame, text = 'Set Pack Completion', padx = 5, pady = 5, bg = '#e8f5e9',
                fg = '#1b5e20', labelanchor = "n")
        set_pack_completion_frame.pack(side = tk.TOP, fill = tk.BOTH, 
            expand = True, padx = 2, pady = 2)
        self.set_pack_completion_frame = tk.Frame(
            set_pack_completion_frame, bg = "#e8f5e9")
        self.set_pack_completion_frame.pack(
            fill = tk.BOTH, expand = True, padx = 5, pady = 2)
        ...
        inv_comp_frame = tk.LabelFrame(
            inv_frame, text = 'Set Completion', padx = 5, pady = 5, bg = '#e3f2fd',
            fg = '#1565c0', labelanchor = 'n')
        inv_comp_frame.pack(side = tk.TOP, fill = tk.BOTH, expand = False, padx = 2, pady = 2)
        self.inv_comp_label = tk.Label(inv_comp_frame, text = '', bg = '#e3f2fd',
            fg = '#1565c0', font = ('Arial', 11, 'bold'), anchor = 'w', justify = 'left')
        self.inv_comp_label.pack(anchor = 'w', padx = 5, pady = 2)

        inv_pack_completion_frame = tk.LabelFrame(
            inv_frame, text = 'Set Pack Completion', padx = 5, pady = 5, bg = '#e3f2fd',
            fg = '#1565c0', labelanchor = "n")
        inv_pack_completion_frame.pack(side = tk.TOP, fill = tk.BOTH, 
            expand = True, padx = 2, pady = 2)
        self.inv_pack_completion_frame = tk.Frame(
            inv_pack_completion_frame, bg = "#e3f2fd")
        self.inv_pack_completion_frame.pack(
            fill = tk.BOTH, expand = True, padx = 5, pady = 2)
        ...
        sug_label = tk.Label(sug_lbl_frame, text = '', font = ('Arial', 12, 'bold'),
            fg = '#1565c0')
    ...
    def _create_status_bar(self):
        ...
        self.status_bar = tk.Label(self, textvariable=self.status_var, 
            bd=1, relief=tk.SUNKEN, anchor="w", font=("Arial", 10), bg="#f5f5f5")
    ...
    def show_dataframe(self, df):
        ...
        self._show_tree(self.tree_set, set_df,
                        self.chart_frame_set, is_set=True)
        self.groups = None
    ...
    def _show_tree(self, tree, df, chart_frame, is_set):
        ...
        # Vectorized check for inventory membership
        ...
        checked_group_id = tree.insert("", tk.END, values=[
            "", "Checked"] + [""] * (len(df.columns) - 1), tags=("checked_group",))
        unchecked_group_id = tree.insert("", tk.END, values=[
            "", "Unchecked"] + [""] * (len(df.columns) - 1), tags=("unchecked_group",))

        # Insert checked items
        for idx, row in checked_data.iterrows():
            values = [u"☑"] + row.tolist()
            item_id = tree.insert(checked_group_id, tk.END,
                                values=values, tags=("item",))
        ...
        for idx, row in unchecked_data.iterrows():
            values = [u"☐"] + row.tolist()
            item_id = tree.insert(unchecked_group_id, tk.END,
                                values=values, tags=("item",))
    ...
    def _populate_tree(self, tree, grouped_data, checkbox_vars, is_set, open_groups=True):
        """
        ...
        if hasattr(grouped_data, 'groups'):  # It's a DataFrameGroupBy object
            for name, indices in grouped_data.groups.items():
                ...
                group_id = tree.insert(
                    "", tk.END, text=f"Group: {name}", values=values, open=open_groups, tags=("group",)
                )
    ...
    def on_group_change(self, value):
        ...
        if value == "Checked":
            ...
            self.groups_all = self._populate_tree(self.tree, groups, self.checkbox_vars_all, is_set=False)
            ...
            self.groups_set = self._populate_tree(self.tree_set, groups_set, self.checkbox_vars_set, is_set=True)
        else:
            # Group by selected column
            grouped = self.df.groupby(value)
            self.groups_all = self._populate_tree(self.tree, grouped, self.checkbox_vars_all, is_set=False)

            set_df = self._get_set_df(self.df)
            grouped_set = set_df.groupby(value)
            self.groups_set = self._populate_tree(self.tree_set, grouped_set, self.checkbox_vars_set, is_set=True)
    ...
    def expand_group(self, item_id, group_df, tree_widget):
        ...
        for idx, row in group_df.iterrows():
            ...
            values = [u"☑" if inv else u"☐"] + [row[col]
                                                for col in self.df.columns]
    ...
    def show_group_bar_chart(self, is_set=False):
        ...
        owned_counts = df[owned_mask][group_col].value_counts().reindex(
            group_counts.index, fill_value=0)
    ...
    def _update_set_completion(self):
        ...
        if "rarity" in self.df.columns:
            ...
            self._clear_set_pack_completion(
                "No rarity data available.", "#e8f5e9")
    ...
    def _update_set_pack_completion(self, set_df):
        ...
        if "pack" in self.df.columns:
            ...
            owned_packs = set_df[set_df.index.isin(
                self.inventory)]["pack"].value_counts()
            ...
            for pack in pack_counts.index:
                ...
                lbl = tk.Label(self.set_pack_completion_frame,
                               text=text, bg=color, font=("Arial", 10), anchor="w")
                lbl.pack(side=tk.TOP, anchor="w", fill=tk.X, padx=2, pady=1)
                self.pack_completion_data.append(
                    (pack, owned, total_pack, missing))

            self.update_pack_suggestion_for_current_tab()
        else:
            self._clear_set_pack_completion(
                "No pack data available.", "#e8f5e9")
            self.pack_completion_data = []
            self.update_pack_suggestion_for_current_tab()
    ...
    def _update_inventory_by_pack(self):
        ...
        if 'pack' in self.df.columns:
            ...
            owned_packs = self.df[self.df.index.isin(
                self.inventory)]['pack'].value_counts()
            ...
            lbl = tk.Label(self.inv_pack_completion_frame,
                           text="No pack data available.", bg="#e3f2fd")
            lbl.pack(side=tk.TOP, anchor="w", fill=tk.X, padx=2, pady=1)
    ...
    def update_pack_suggestion(self, is_set=False):
        ...
        if not packs:
            self._set_suggest_label(
                "Congratulations! All packs are complete.", is_set)
            return
        ...
        missing_cards = df[(~df.index.isin(self.inventory))
                           & (df.index.isin(relevant_indices))]
        missing_cards = missing_cards[missing_cards["rarity"].notna(
        ) & missing_cards["pack"].notna()]
        ...
        if not pack_probs:
            self._set_suggest_label(
                "No probability data available.", is_set)
            return
    ...
    def _calculate_pack_probabilities(self, packs, missing_cards, rarity_to_row):
        ...
        prob_matrix = pl.from_dict(prob_matrix).transpose()
        ...
        for pack_info in packs:
            ...
            if len(row_indices) > 0:
                row_p = 1 - prob_matrix[row_indices].to_numpy()
                pack_probs[pack_name] = np.sum(1 - np.power(row_p[:,0], 3) * np.prod(row_p[:, 1:], axis=1))
    ...
    def _download_all_set_images_with_progress(self, progress_var, progress_label, top):
        ...
        no_cap_sets, all_sets = self._get_local_set_names(folder=self.sets_folder)  # Get sets from 'sets' folder
        total_cards = 0
        ...
        for json_name, set_name in zip(no_cap_sets, all_sets):
            ...
            try:
                with open(json_path, 'r', encoding='utf-8') as f:
                    set_data = json.load(f)
                set_df = pd.DataFrame(set_data)  # Create DataFrame from JSON data
                ...
                for index, row in set_df.iterrows():
                    ...
                    if not os.path.exists(local_path):
                        url = img_aqcuisition.get_image(
                            card_name=card_name, card_id=card_id, set_name=set_name, card_rarity=card_rarity
                        )
                        if url:
                            try:
    ...
    def display_card_image(self, idx, widget):
        ...
        card_id = card_id.split('-')[1].lstrip('0') if '-' in card_id else card_id
        card_rarity = row.get("rarity", "")
        set_name = row.get("set", "").replace(' ', '_')  # Get set and prepare for path
        label = self.img_label_set if widget == self.tree_set else self.img_label
        label.config(image="", text="Loading image...")
        label.image = None

        pil_image, photo = self._load_local_image(card_name, card_id, set_name, card_rarity)

        if pil_image:  # Check if pil_image was loaded successfully
            container_width, container_height = 480, 420
            original_width, original_height = pil_image.size
            width_scale = container_width / original_width
            height_scale = container_height / original_height
